...
class WatermarkConfig(QWidget):
    ...
    def _parse_wm_name(stem):
        ...
        - 去序号：img-wm-bits（仅出现在双段叠加名的后续段里，如
          001-GC-密钥_7-9-96 的 7-9-96）——单段名不认此形态，
          防止 2024-01-01 之类的纯数字名被误认。
        混淆段第二位恒为 GC/RO/BS 字母，不会被全数字段判定误认。"""
        parts = stem.split('_')
        multi = len(parts) > 1  # 去序号形态仅在叠加名（含 '_'）中合法
        for part in parts:
            segs = part.split('-')
            if len(segs) == 4 and all(s.isdigit() for s in segs):
                return int(segs[1]), int(segs[2]), int(segs[3])
            if multi and len(segs) == 3 and all(s.isdigit() for s in segs):
