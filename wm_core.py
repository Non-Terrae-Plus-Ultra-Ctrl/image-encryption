# -*- coding: utf-8 -*-
"""wm_core.py — 盲水印算法核心（DCT-SVD 分块频域水印）。

融合自开源库 blind_watermark（github.com/guofei9987/blind_watermark，
MIT License，Copyright (c) 2019 郭飞），按本项目实际用量合并为单文件：
- WaterMark      用户级封装：读图/读水印（文字·位图·图片）/嵌入/提取
- WaterMarkCore  DCT-SVD 分块嵌入与提取核心（haar 小波 → DCT → SVD 奇异值量化）
- AutoPool       分块批处理池（common 顺序映射，Windows 兼容）
- imread/imwrite_unicode  中文/非 ASCII 路径读写（np.fromfile + imdecode）
未用到的攻击测试(att)、多进程后端、命令行工具已裁剪。

用法：from wm_core import WaterMark
"""
...
# ---------------------------------------------------------------------------
# Unicode 路径读写（cv2.imread/imwrite 在 Windows 下不支持非 ASCII 路径）
# ---------------------------------------------------------------------------
def imread_unicode(filename):
    """读取图片，支持中文/Unicode 路径；用 IMREAD_UNCHANGED 保留 alpha。"""
...
def imwrite_unicode(filename, img):
    """写出图片，支持中文/Unicode 路径（imencode + tofile）。"""
...
# ---------------------------------------------------------------------------
# AutoPool：分块批处理（common=顺序映射；Windows 下不启用多进程）
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# DCT-SVD 分块核心：haar 小波低频 → 4x4 分块 → DCT → SVD 奇异值量化嵌水印
# ---------------------------------------------------------------------------
class WaterMarkCore:
    ...
    def read_img(self, filename):
        ...
        # 若带 alpha（透明 PNG），单独保留，嵌入只处理 BGR 三通道
    ...
    def embed(self, filename):
        ...
        # 若原图带 alpha（透明 PNG），把透明通道拼回去，避免透明背景被覆盖
        if getattr(self, 'alpha', None) is not None:
            embed_img = np.dstack([embed_img, self.alpha])
...
class WaterMark:
    ...
    def read_img_wm(self, filename):
        ...
        # 灰度图读入是 2 维数组，统一转 3 维再取通道，避免 wm[:, :, 0] 越界
        if wm.ndim == 2:
